...
public class OrdSet3_9 extends CaseStudy{
	 ...
	 * @return 
	 */
	public OrdSet3_9 (Integer size) {
		try {
			_set_size = defSetSize(size);
			_set = new int[_set_size];
			_last = -1;
			_resized_times = _set_size/min_set_size -1;
			_overflow = false;
			currentState = "Empty";
		} catch (Exception e) {
			currentState = "Failed";
	 ...
	 */
	public boolean contains (OrdSet3_9 x) {
		int k, n;
...
being made.
	 ...
	 */
	public OrdSet3_9 union(OrdSet3_9 s2) {
		...
		OrdSet3_9 set = new OrdSet3_9(size1 + size2);
	...
	private int outputDefSetSize() {
		if (_set_size < min_set_size) return min_set_size;
		else if (_set_size >= max_set_size) return OrdSet3_9.max_set_size;
		else if (_set_size == (_set_size/min_set_size)*min_set_size) return _set_size;
		else return ((_set_size/min_set_size)+1)*min_set_size;
	}
	
	private boolean isPostOrdSet() {
		if (getSetSize()== outputDefSetSize()
				&& getResizedTimes() == 0
				&& !isOverflow()
				&& getSetLast() == -1
				&& getSetElements().length == 0) return true;
		else return false;
	}

	private boolean isElement(int[] v, int n) {
	...
	private int outputDefSetSize(int size) {
		if (size < min_set_size) return min_set_size;
		else if (size >= max_set_size) return max_set_size;
		else if (size == (size/min_set_size)*min_set_size) return size;
		else return ((size/min_set_size)+1)*min_set_size;
	}

	private boolean isPostOrdSet(int[] v) {
		boolean flag = true;
		int[] set = getSetElements();
		for (int i=0; i<getSetLast(); i++) {
			if (!isElement(v, set[i])) flag = false;
			if (!flag) break;
		}
		if (getSetSize() == outputDefSetSize(v.length)
				&& getSetLast() < v.length && flag ) 
			return true;
		else 
			return false;
	}
	...
	public void setCurrentState() {
		if( _last < 0)
			...
			currentState = "PF2";
		else if ((_last==2) && (this._resized_times>0))
			currentState = "PF3";
		else if ((_last==2) && (this._resized_times==0))
			currentState = "F3";
		else if (_last==3)
			...
			currentState = "PF5";
		else if ((_last==5) && (this._resized_times>1))
			currentState = "PF6";
		else if ((_last==5) && (this._resized_times<=1))
			currentState = "F6";	
		else if (_last==6)
		...
		else if ((_last==8)&&(this._resized_times<=2))
			currentState = "F9";
		else
			currentState = "Failed";
	}
